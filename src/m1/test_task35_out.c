...
State conn;
  int _m1_ret = 0;
  _m1_ret = ((conn = Closed, ({ (conn = Open); conn; }), (0, (0, ({ if (conn.Connecting) { "connecting now"; } 0; })))));
  goto __m1_return;
__m1_return:
if (!_will_0) { fprintf(stderr, "<unknown>:5:9: error: [M1102] 'will' commitment violated\n"); exit(1); } if (!_will_1) { fprintf(stderr, "<unknown>:6:9: error: [M1102] 'will' commitment violated\n"); exit(1); }   return _m1_ret;
}
