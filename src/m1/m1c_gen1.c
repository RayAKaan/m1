...
extern int64_t TK_RETURN;
extern int64_t TK_BREAK;
extern int64_t TK_CONTINUE;
extern int64_t TK_MODULE_DECL;
extern int64_t TK_IMPORT;
...
extern int64_t phase_graph_init;
...
M1LexResult r;
  PS _m1_ret = {0};
  _m1_ret = (r = m1_lexer_next(s.lx), (PS){ .tk = r.token, .lx = r.lexer, .err = s.err });
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
int64_t _will_0=0,_will_1=0,_will_2=0,_will_3=0,_will_4=0,_will_5=0,_will_6=0,_will_7=0,_will_8=0,_will_9=0;
  PS _m1_ret = {0};
  _m1_ret = (PS){ .tk = s.tk, .lx = s.lx, .err = (s.err + 1) };
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
PR r2;
  PR _m1_ret = {0};
  _m1_ret = (r = M1C__parse_type_app(s), (fprintf(stderr, "PTA: kind=%lld TK_ARROW=%lld\n", (long long)M1C__ps_kind(r.st), (long long)TK_ARROW), ((M1C__ps_kind(r.st) == TK_ARROW) ? (r2 = M1C__parse_type_arrow(M1C__ps_adv(r.st)), M1C__mk_pr(M1C__t_fn(m0_cons(r.node, m0_nil), 1, r2.node, M1C__ps_line(r.st), M1C__ps_col(r.st)), r2.st)) : (r))));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
char* cn;
  PR _m1_ret = {0};
  _m1_ret = (k = M1C__ps_kind(s), line = M1C__ps_line(s), col = M1C__ps_col(s), ((k == TK_TYPE) ? (nm = M1C__ps_str(M1C__ps_adv(s)), s2 = M1C__ps_adv(M1C__ps_adv(s)), tr = M1C__parse_type_expr(s2), M1C__mk_pr(M1C__n_type_decl(nm, tr.node, line, col), tr.st)) : ((k == TK_FN) ? (nm = M1C__ps_str(M1C__ps_adv(s)), pr = M1C__gather_params(M1C__ps_adv(M1C__ps_adv(s)), m0_nil), s2 = M1C__ps_adv(pr.st), rr = M1C__parse_type_expr(s2), s3 = M1C__ps_adv(rr.st), br = M1C__parse_expr(s3), s4 = M1C__ps_adv(br.st), M1C__mk_pr(M1C__n_fn_decl(nm, pr.node, M1C__list_len(pr.node), rr.node, br.node, line, col), s4)) : ((k == TK_EXTERNAL) ? (cn = M1C__ps_str(M1C__ps_adv(s)), s2 = M1C__ps_adv(M1C__ps_adv(M1C__ps_adv(s))), tr = M1C__parse_type_expr(s2), M1C__mk_pr(M1C__n_extern(cn, tr.node, line, col), tr.st)) : ((k == TK_LET) ? (nm = M1C__ps_str(M1C__ps_adv(s)), s2 = M1C__ps_adv(M1C__ps_adv(M1C__ps_adv(s))), tr = M1C__parse_type_expr(s2), s3 = M1C__ps_adv(tr.st), br = M1C__parse_expr(s3), M1C__mk_pr(M1C__n_let(nm, tr.node, br.node, M1C__n_unit(line, col), line, col), br.st)) : (M1C__mk_pr(M1C__n_unit(line, col), M1C__ps_err(s))))))));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
PR r;
  PR _m1_ret = {0};
  _m1_ret = (((M1C__ps_kind(s) == TK_RBRACE) ? (M1C__mk_pr(M1C__rev(acc), M1C__ps_adv(s))) : (((M1C__ps_kind(s) == TK_EOF) ? (M1C__mk_pr(M1C__rev(acc), M1C__ps_err(s))) : (((s.err) ? (M1C__mk_pr(M1C__rev(acc), s)) : (r = M1C__parse_decl(s), M1C__gather_decls(r.st, m0_cons(r.node, acc)))))))));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
PR dr;
  PR _m1_ret = {0};
  _m1_ret = (line = M1C__ps_line(s), col = M1C__ps_col(s), nm = M1C__ps_str(M1C__ps_adv(s)), s2 = M1C__ps_adv(M1C__ps_adv(M1C__ps_adv(s))), dr = M1C__gather_decls(s2, m0_nil), M1C__mk_pr(M1C__n_mod(nm, dr.node, M1C__list_len(dr.node), line, col), dr.st));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
PR r;
  PR _m1_ret = {0};
  _m1_ret = (((M1C__ps_kind(s) == TK_EOF) ? (M1C__mk_pr(M1C__rev(acc), s)) : (((M1C__ps_kind(s) == TK_MODULE) || (M1C__ps_kind(s) == TK_MODULE_DECL)) ? (r = M1C__parse_module(s), M1C__gather_modules(r.st, m0_cons(r.node, acc))) : (M1C__mk_pr(M1C__rev(acc), M1C__ps_err(s))))));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
int64_t _p5;
  int64_t _m1_ret = 0;
  _m1_ret = (((m0_is_nil(l) == 1) ? (0) : (d = m0_head(l), _e = ((m0_get(d, 0) == N_TYPE_DECL) ? (M1C__emit_typedef(reg, m0_get(d, 4), m0_load_string(d, 3))) : ((m0_get(d, 0) == N_LET) ? (_p0 = M1C__emit_c_type(reg, m0_get(d, 4)), _p1 = m0_print(" "), _p2 = m0_print(m0_load_string(d, 3)), _p3 = m0_print(" = "), _p4 = M1C__emit_c_expr(reg, m0_get(d, 5)), _p5 = m0_print(";\n")) : (0))), M1C__emit_module_pass1(reg, m0_tail(l)))));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
...
int64_t _em;
  int _m1_ret = 0;
  _m1_ret = (filename = m0_get_env("M1_SOURCE"), ((m0_string_eq(filename, "") == 1) ? (_e = m0_print("[M1000] error: no source file\n"), 1) : (src = m0_read_file(filename), l = m1_lexer_init(src), r = m1_lexer_next(l), s = (PS){ .tk = r.token, .lx = r.lexer, .err = 0 }, pr = M1C__parse_program(s), mods = m0_get(pr.node, 3), _em = ((m0_is_nil(mods) == 0) ? (M1C__emit_module(m0_head(mods))) : (0)), pr.st.err)));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
