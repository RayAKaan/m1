...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Idle = 0, Active = 1, Committed = 2, RolledBack = 3, tx = Idle, _ = ({ if (!((tx >= 0))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task6_transaction.m1:86:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = /* will tx.Committed satisfied */ 1, _ = ({ (tx = Active); tx; }), _ = say(int_to_string(1)), _ = ({ (tx = Committed); tx; }), _ = say(int_to_string(1)), _ = 0, _ = say(int_to_string(1)), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
