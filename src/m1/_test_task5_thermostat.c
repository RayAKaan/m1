...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (temp = 22, _ = ({ if (!((temp >= 15))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task5_thermostat.m1:81:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = ({ if ((temp > 30)) { say("cooling"); } 0; }), _ = ({ if ((temp < 15)) { say("heating"); } 0; }), _ = ({ (temp = 18); if ((temp > 30)) { say("cooling"); } if ((temp < 15)) { say("heating"); } temp; }), _ = ({ (temp = 32); if ((temp > 30)) { say("cooling"); } if ((temp < 15)) { say("heating"); } temp; }), _ = ({ (temp = 10); if ((temp > 30)) { say("cooling"); } if ((temp < 15)) { say("heating"); } temp; }), _ = say(int_to_string(temp)), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
