...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Draft = 0, Confirmed = 1, Shipped = 2, Delivered = 3, order = Draft, _ = say(int_to_string(1)), _ = ({ (order = Confirmed); order; }), _ = say(int_to_string(1)), _ = say(int_to_string(1)), _ = ({ (order = Shipped); order; }), _ = say(int_to_string(1)), _ = ({ (order = Delivered); order; }), _ = say(int_to_string(1)), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
