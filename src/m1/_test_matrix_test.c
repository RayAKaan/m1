...
int64_t j;
  int _m1_ret = 0;
  _m1_ret = (m = matrix_new(3, 3), i = 0, _ = ({ while ((i < 3)) { j = 0, _ = ({ while ((j < 3)) { _ = matrix_set(m, i, j, (((i * 3) + j) + 1)), ({ (j = (j + 1)); j; }); } 0; }), ({ (i = (i + 1)); i; }); } 0; }), _ = say(int_to_string(matrix_get(m, 0, 0))), _ = say(int_to_string(matrix_get(m, 1, 1))), say(int_to_string(matrix_get(m, 2, 2))));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
