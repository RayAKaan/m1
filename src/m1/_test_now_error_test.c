...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (balance = (0 - 1), _ = ({ if (!((balance >= 0))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_now_error_test.m1:84:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), say(int_to_string(balance)));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
