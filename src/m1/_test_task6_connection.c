...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Offline = 0, Online = 1, conn = Online, _ = ({ if ((conn == Offline)) { say("disconnected"); } 0; }), _ = ({ if ((conn == Offline)) { _will_0 = 1; } 0; }), _ = ({ (conn = Offline); if ((conn == Offline)) { say("disconnected"); } if ((conn == Offline)) { _will_0 = 1; } conn; }), say("done"));
  goto __m1_return;
__m1_return:
if (!_will_0) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task6_connection.m1:84:23: error: [M1102] 'will' commitment violated\n"); exit(1); }   return _m1_ret;
}
