...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (order = 0, _ = ({ if ((order == 1)) { _will_0 = 1; } 0; }), _ = ({ (order = 2); if ((order == 1)) { _will_0 = 1; } order; }), 0);
  goto __m1_return;
__m1_return:
if (!_will_0) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_will_error_test.m1:81:23: error: [M1102] 'will' commitment violated\n"); exit(1); }   return _m1_ret;
}
