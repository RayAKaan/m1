...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Closed = 0, Open = 1, file = Closed, _ = ({ if (!((file >= 0))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task3_file.m1:84:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = 0, _ = ({ (file = Open); _will_0 = 1; file; }), _ = say(int_to_string(1)), _ = ({ if ((file == Closed)) { say("closed"); } 0; }), _ = ({ (file = Closed); if ((file == Closed)) { say("closed"); } _will_0 = 1; file; }), 0);
  goto __m1_return;
__m1_return:
if (!_will_0) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task3_file.m1:85:23: error: [M1102] 'will' commitment violated\n"); exit(1); }   return _m1_ret;
}
