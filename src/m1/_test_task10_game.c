...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Setup = 0, Playing = 1, Paused = 2, GameOver = 3, state = Setup, score = 0, _ = ({ if (!((score >= 0))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task10_game.m1:87:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = ({ if ((score >= 100)) { say("win"); } 0; }), _ = ({ (state = Playing); state; }), _ = say(int_to_string(1)), _ = ({ (score = 50); if ((score >= 100)) { say("win"); } score; }), _ = ({ (score = 100); if ((score >= 100)) { say("win"); } score; }), _ = ({ (state = GameOver); state; }), _ = say(int_to_string(1)), _ = say(int_to_string(score)), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
