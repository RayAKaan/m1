...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Closed = 0, Open = 1, doors = Closed, floor = 1, _ = ({ if (!((floor >= 1))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task7_elevator.m1:85:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = ({ if (!((floor <= 20))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task7_elevator.m1:86:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = ({ if ((doors == Open)) { say("doors open"); } 0; }), _ = ({ (floor = 5); floor; }), _ = ({ (doors = Open); if ((doors == Open)) { say("doors open"); } doors; }), _ = say(int_to_string(1)), _ = say(int_to_string(floor)), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
