...
int64_t sum;
  int _m1_ret = 0;
  _m1_ret = (s = "Hello, World", _ = say(int_to_string(char_at(s, 0))), _ = say(substring(s, 7, 5)), parts = string_split("10 20 30", " "), i = 0, sum = 0, _ = ({ while ((i < vector_len(parts))) { _ = ({ (sum = (sum + string_to_int(vector_get_str(parts, i)))); sum; }), ({ (i = (i + 1)); i; }); } 0; }), say(int_to_string(sum)));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
