...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Guest = 0, Authenticated = 1, Admin = 2, session = Guest, _ = ({ if (!((session >= 0))) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task4_session.m1:85:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = say(int_to_string(1)), _ = ({ (session = Authenticated); session; }), _ = say(int_to_string(1)), _ = ({ if ((session == Admin)) { say("admin activated"); } 0; }), _ = ({ (session = Admin); if ((session == Admin)) { say("admin activated"); } session; }), _ = say(int_to_string(1)), _ = 0, _ = say(int_to_string(1)), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
