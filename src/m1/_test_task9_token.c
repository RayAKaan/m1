...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (Issued = 0, Used = 1, Expired = 2, token = Issued, _ = 0, _ = say(int_to_string(1)), _ = ({ (token = Used); _will_0 = 1; token; }), _ = say(int_to_string(1)), _ = ({ (token = Expired); _will_0 = 1; token; }), _ = say(int_to_string(1)), 0);
  goto __m1_return;
__m1_return:
if (!_will_0) { fprintf(stderr, "H:/AI-Focused_language/src/m1/_inj_task9_token.m1:85:23: error: [M1102] 'will' commitment violated\n"); exit(1); }   return _m1_ret;
}
