...
int64_t j;
  int _m1_ret = 0;
  _m1_ret = (v = vector_new(), i = 0, _ = ({ while ((i < 5)) { _ = vector_push(v, (5 - i)), ({ (i = (i + 1)); i; }); } 0; }), _ = vector_sort(v), j = 0, _ = ({ while ((j < vector_len(v))) { _ = say(int_to_string(vector_get(v, j))), ({ (j = (j + 1)); j; }); } 0; }), 0);
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
