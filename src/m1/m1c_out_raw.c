...
return m0_print(")");                  
                                        ...
                                        if 
  (k == N_LET) {                      
     /* record phase for initial binding (mutable set) */
     { char* lnm = m0_load_string(e, 3);
       int64_t lval = m0_get(e, 5);
       if (m0_get(lval, 0) == N_NAME) {
         char* lrhs = m0_load_string(lval, 3);
         phase_record(lnm, lrhs);
       }
       phase_record(lnm, "live");
     }
...
(k == N_WHILE) {
 ...
 (k == N_WHEN) {
  ...
  (k == N_WILL) {
                                        int64_t cond = m0_get(e, 3);
                                        char* watched = extract_watched_var(cond);
                                        /* Phase Graph: try compile-time will folding first */
                                        if (watched && watched[0] != '\0' && is_phase_will(cond)) {
                                            char* phase = extract_phase_name(cond);
                                            int pgw = phase_graph_will_query(watched, phase);
                                            if (pgw == 1) {
                                                m0_print("/* will ");
                                                m0_print(watched);
                                                m0_print(".");
                                                m0_print(phase);
                                                m0_print(" satisfied */ 1");
                                                return 0;
                                            } else if (pgw == 0) {
                                                /* will can never be satisfied – still emit runtime
                                                   to catch the violation error gracefully */
                                            }
                                        }
                                        if (watched && watched[0] != '\0') {
...
static int64_t let_declared(const char *nm) {
    for (int64_t _di = 0; _di < g_export_count; _di++)
        if (g_export_modname[_di] == (char*)1 && m0_string_eq(g_export_fnname[_di], (char*)nm))
            return 1;
    return 0;
}
...
int64_t emit_let_decls(int64_t e) {
  ...
  if (k == N_LET) {
    char* nm = m0_load_string(e, 3);
    if (!let_declared(nm) && g_export_count < MAX_EXPORTS) {
...
m0_print("int64_t _will_0=0,_will_1=0,_will_2=0,_will_3=0,"
  ...
  int64_t saved_export_count = g_export_count;
  emit_let_decls(body);
...
m0_print(";\n  goto __m1_return;\n__m1_return:\n");  (void)p7;
  ...
  g_export_count = saved_export_count;
          ...
          if (m0_get(m, 0) == N_MOD) {
            if (!header_printed) {
              m0_print("#include <stdio.h>\n#include <stdint.h>\n#include <stdlib.h>\n\n");
               ...
               m0_print("int64_t m0_print(char* s);\n");
               m0_print("int64_t m0_println(char* s);\n");
              ...
              m0_print("int64_t read_int(void);\n");
               m0_print("int64_t char_at(char* s, int64_t i);\n");
               m0_print("char* substring(char* s, int64_t start, int64_t len);\n");
               m0_print("int64_t string_split(char* s, char* delim);\n");
              ...
              m0_print("char* float_to_string(int64_t f);\n");
               m0_print("int64_t string_to_float(char* s);\n\n");
              header_printed = 1;
            }
            emit_module(m);
          }
