...
int64_t _;
  int _m1_ret = 0;
  _m1_ret = (balance = 1000, _ = ({ if (!((balance >= 0))) { fprintf(stderr, "H:/AI-Focused_language/tests/benchmark/task1_bank.m1:4:23: error: [M1103] 'now' assertion violated: (invariant)\n"); exit(1); } 0; }), _ = ({ if ((balance < 100)) { say("low balance"); } 0; }), _ = ({ (balance = (balance - 500)); if ((balance < 100)) { say("low balance"); } balance; }), say(int_to_string(balance)));
  goto __m1_return;
__m1_return:
  return _m1_ret;
}
